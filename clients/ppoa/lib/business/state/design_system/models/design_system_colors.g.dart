// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'design_system_colors.dart';

// **************************************************************************
// JsonSerializableGenerator
// **************************************************************************

_$_DesignSystemColors _$$_DesignSystemColorsFromJson(Map<String, dynamic> json) => _$_DesignSystemColors(
      teal: colorFromJson(json['teal'] as String),
      purple: colorFromJson(json['purple'] as String),
      green: colorFromJson(json['green'] as String),
      yellow: colorFromJson(json['yellow'] as String),
      pink: colorFromJson(json['pink'] as String),
      white: colorFromJson(json['color_white'] as String),
      black: colorFromJson(json['color_black'] as String),
      colorGray1: colorFromJson(json['color_gray1'] as String),
      colorGray2: colorFromJson(json['color_gray2'] as String),
      colorGray3: colorFromJson(json['color_gray3'] as String),
      colorGray4: colorFromJson(json['color_gray4'] as String),
      colorGray6: colorFromJson(json['color_gray6'] as String),
      colorGray7: colorFromJson(json['color_gray7'] as String),
    );

Map<String, dynamic> _$$_DesignSystemColorsToJson(_$_DesignSystemColors instance) => <String, dynamic>{
      'teal': colorToJson(instance.teal),
      'purple': colorToJson(instance.purple),
      'green': colorToJson(instance.green),
      'yellow': colorToJson(instance.yellow),
      'pink': colorToJson(instance.pink),
      'color_white': colorToJson(instance.white),
      'color_black': colorToJson(instance.black),
      'color_gray1': colorToJson(instance.colorGray1),
      'color_gray2': colorToJson(instance.colorGray2),
      'color_gray3': colorToJson(instance.colorGray3),
      'color_gray4': colorToJson(instance.colorGray4),
      'color_gray6': colorToJson(instance.colorGray6),
      'color_gray7': colorToJson(instance.colorGray7),
    };
