// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'api.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$systemApiServiceHash() => r'55dba5d4744a1cff7fc553d15873331e9b389e72';

/// See also [systemApiService].
@ProviderFor(systemApiService)
final systemApiServiceProvider = FutureProvider<SystemApiService>.internal(
  systemApiService,
  name: r'systemApiServiceProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$systemApiServiceHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef SystemApiServiceRef = FutureProviderRef<SystemApiService>;
String _$postApiServiceHash() => r'0cb0770e6cdf4244f6ddf24ebc9b1e71eafb6613';

/// See also [postApiService].
@ProviderFor(postApiService)
final postApiServiceProvider = FutureProvider<PostApiService>.internal(
  postApiService,
  name: r'postApiServiceProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$postApiServiceHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef PostApiServiceRef = FutureProviderRef<PostApiService>;
String _$profileApiServiceHash() => r'868de48f2860a17d4e46bdd7bd71738231ae772e';

/// See also [profileApiService].
@ProviderFor(profileApiService)
final profileApiServiceProvider = FutureProvider<ProfileApiService>.internal(
  profileApiService,
  name: r'profileApiServiceProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$profileApiServiceHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef ProfileApiServiceRef = FutureProviderRef<ProfileApiService>;
String _$relationshipApiServiceHash() =>
    r'7885719698ae96db87dfc8911f64b18e6f6e663c';

/// See also [relationshipApiService].
@ProviderFor(relationshipApiService)
final relationshipApiServiceProvider =
    FutureProvider<RelationshipApiService>.internal(
  relationshipApiService,
  name: r'relationshipApiServiceProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$relationshipApiServiceHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef RelationshipApiServiceRef = FutureProviderRef<RelationshipApiService>;
String _$conversationApiServiceHash() =>
    r'e5f16a2ca70f4ea7fc5a95d50d472767a2251a6d';

/// See also [conversationApiService].
@ProviderFor(conversationApiService)
final conversationApiServiceProvider =
    FutureProvider<ConversationApiService>.internal(
  conversationApiService,
  name: r'conversationApiServiceProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$conversationApiServiceHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef ConversationApiServiceRef = FutureProviderRef<ConversationApiService>;
String _$guidanceApiServiceHash() =>
    r'3ccf694dbd676082714a05e6d306941727857c21';

/// See also [guidanceApiService].
@ProviderFor(guidanceApiService)
final guidanceApiServiceProvider = FutureProvider<GuidanceApiService>.internal(
  guidanceApiService,
  name: r'guidanceApiServiceProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$guidanceApiServiceHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef GuidanceApiServiceRef = FutureProviderRef<GuidanceApiService>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member
