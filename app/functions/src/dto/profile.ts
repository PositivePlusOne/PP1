...
export const allVisibilityFlags = [visibilityFlagName, visibilityFlagBirthday, visibilityFlagIdentity, visibilityFlagInterests, visibilityFlagGenders, visibilityFlagLocation, visibilityFlagHivStatus, visibilityFlagCompanySectors];
...
export class Profile {
  ...
  computeSearchTags(): void {
    this._tags = [this.displayName.length > 0 ? "hasDisplayName" : "", this.media.filter((media) => !media.isPrivate).length > 0 ? "hasPublicMedia" : ""];
  }
