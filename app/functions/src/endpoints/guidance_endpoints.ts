...
export namespace GuidanceEndpoints {
  export const getGuidanceCategories = functions
    .region("europe-west3")
  ...
  export const getGuidanceArticles = functions
    ...
    .https.onCall(async (data: any, context) => {
      ...
      let query = firestore.collection("fl_content").where("_fl_meta_.schema", "==", "guidanceArticles").where("locale", "==", locale);
  ...
  export const getGuidanceDirectoryEntries = functions
    .region("europe-west3")
    .runWith(FIREBASE_FUNCTION_INSTANCE_DATA)
    .https.onCall(async (data: EndpointRequest, context) => {
      await SystemService.validateUsingRedisUserThrottle(context);
