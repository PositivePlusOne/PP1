...
export namespace EventEndpoints {
  export const importTestEvents = functions.runWith(FIREBASE_FUNCTION_INSTANCE_DATA).https.onCall(async (data) => {
    ...
    const activities = ActivityMappers.convertOccasionGeniusEventsToActivities(suppliedEvents.results ?? []);

    const profileKey = process.env.POSITIVEPLUSONE_PROFILE_KEY || "";
    if (!profileKey || profileKey === "") {
      throw new functions.https.HttpsError("internal", "Profile key not set");
    }

    const organisationProfile = await ProfileService.getProfile(profileKey);
