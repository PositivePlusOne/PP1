...
export namespace PostSharedNotification {
  /**
  ...
  export async function sendNotification(userProfile: ProfileJSON, targetProfile: ProfileJSON, activity: ActivityJSON): Promise<void> {
    ...
    const title = await LocalizationsService.getLocalizedString("notifications.post_shared.title");

    const safeContent = StringHelpers.markdownToPlainText(content);
    const body = await LocalizationsService.getLocalizedString("notifications.post_shared.body", { displayName, shortBody: safeContent });
    ...
    const payload = new NotificationPayload({
      ...
      action: NotificationAction.POST_SHARED,
    });
