...
export namespace ReactionGroupedLikedNotification {
  /**
  ...
  export async function sendNotification(targetProfile: ProfileJSON, activity: ActivityJSON, reaction: ReactionJSON, reactionStatistics: ReactionStatisticsJSON): Promise<void> {
    ...
    const title = await LocalizationsService.getLocalizedString("notifications.post_liked_grouped.title");
    const body = hasActivityContent ? await LocalizationsService.getLocalizedString("notifications.post_liked_grouped.body", { shortBody: activityContent, count }) : await LocalizationsService.getLocalizedString("notifications.post_liked_grouped.body_empty", { count });
    ...
    const identifierParts = [NotificationAction.POST_LIKED_GROUP, receiverId, activityId];
    ...
    const payload = new NotificationPayload({
      ...
      action: NotificationAction.POST_LIKED_GROUP,
    });
