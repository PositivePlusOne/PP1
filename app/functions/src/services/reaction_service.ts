...
export namespace ReactionService {
  ...
  export async function addReaction(client: StreamClient<DefaultGenerics>, reaction: ReactionJSON): Promise<[ReactionJSON, ReactionStatisticsJSON, ProfileStatisicsJSON, ProfileStatisicsJSON]> {
    ...
    const [newReactionStats, newSourceProfileStats, newTargetProfileStats] = await Promise.all([ReactionStatisticsService.updateReactionCountForActivity(expectedActivityId, expectedKind, 1), ProfileStatisticsService.updateReactionCountForProfile(expectedUserId, expectedKindGiven, 1), ProfileStatisticsService.updateReactionCountForProfile(targetUserId || "", expectedKindReceived, 1)]);
  ...
  export async function deleteReaction(client: StreamClient<DefaultGenerics>, reaction: ReactionJSON): Promise<[ReactionStatisticsJSON, ProfileStatisicsJSON, ProfileStatisicsJSON]> {
    ...
    const [newReactionStats, newSourceProfileStats, newTargetProfileStats] = await Promise.all([ReactionStatisticsService.updateReactionCountForActivity(reaction.activity_id, reaction.kind, -1), ProfileStatisticsService.updateReactionCountForProfile(reaction.target_user_id || "", expectedKindReceived, -1), ProfileStatisticsService.updateReactionCountForProfile(reaction.user_id, expectedKindGiven, -1)]);
