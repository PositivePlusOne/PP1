...
export namespace StreamService {
  ...
  /**
   * Rejects an invitation for the given channel.
   * @param {string} channelName the name of the channel.
   * @param {any} userProfile the user profile.
   * @return {Channel<DefaultGenerics>} the rejected channel.
   */
  export async function rejectInvitation(
    channelName: string,
    userProfile: any
  ): Promise<Channel<DefaultGenerics>> {
    functions.logger.info("Rejecting invitation", { channelName, userProfile });
    const streamInstance = getStreamInstance();
    const uid = userProfile._fl_meta_.docId;
