...
export namespace PromoteActivityAction {
  export async function promoteActivity(action: AdminQuickActionJSON): Promise<void> {
    ...
    const [sourceProfileSnapshot, targetActivitySnapshot, targetPromotionSnapshot] = await Promise.all([sourceProfile.profiles![0].get(), targetActivity.activities![0].get(), targetPromotion.promotions![0].get()]);
