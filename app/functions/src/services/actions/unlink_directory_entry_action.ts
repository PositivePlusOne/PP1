...
export namespace UnlinkDirectoryEntryAction {
  export async function unlinkDirectoryEntry(action: AdminQuickActionJSON): Promise<void> {
    ...
    const [sourceProfileSnapshot, directoryEntrySnapshot] = await Promise.all([sourceReference.get(), directoryEntryReference.get()]);
    ...
    await Promise.all([sourceReference.update({ _fl_meta_: profileMeta }), directoryEntryReference.update({ _fl_meta_: directoryEntryMeta })]);
